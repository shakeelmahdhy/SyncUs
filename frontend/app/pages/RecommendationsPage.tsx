...
function bandMatches(job: Job, band: ScoreBand) {
...
export function RecommendationsPage() {
  ...
  const [savedIds, setSavedIds] = useState<number[]>([jobs[0]?.id].filter(Boolean));

  const recommendedJobs = useMemo(() => {
    const normalisedQuery = query.trim().toLowerCase();

    return jobs
      .filter((job) => job.recommended || job.matchScore >= 80)
      .filter((job) => {
        const matchesQuery =
          !normalisedQuery ||
      ...
      <header className="mb-8 grid gap-6 lg:grid-cols-[1fr_340px] lg:items-end">
        <div>
          ...
          <p className="mt-4 max-w-2xl text-base font-medium text-syncus-blue/62">
            Roles ranked by skills, work preferences, location fit, and recent activity from your profile.
          </p>
        </div>
        ...
        <section className="rounded-2xl border-2 border-syncus-blue/20 bg-syncus-cream p-5 shadow-card">
          ...
          <p className="mt-2 text-5xl font-black leading-none text-syncus-green">{topScore}%</p>
          <p className="mt-2 text-sm font-medium text-syncus-blue/55">
            {recommendedJobs[0]?.title ?? "No matching roles found"}
          </p>
        </section>
      </header>
      ...
      <section className="grid gap-5">
            ...
            >
              ...
              <div className="mt-5 flex flex-col gap-4 border-t border-syncus-green/25 pt-4 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex flex-wrap gap-2">
                  {[job.workType, job.locationMode, job.salary, ...job.skills.slice(0, 3)].map((tag) => (
        ...
        {recommendedJobs.length === 0 && (
          <section className="rounded-2xl border-2 border-dashed border-syncus-green p-12 text-center text-syncus-blue">
            ...
            <p className="mt-2 text-sm font-medium text-syncus-blue/55">Clear the search or lower the match threshold.</p>
          </section>
        )}
      </section>
    </main>
